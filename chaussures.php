        ...
        <div class="shop_products_lists">
            ...
            <section class="shop_page_products">
                <div class="shop_page_products_lists">
                    <?php foreach ($this->chaussuresPagination as $chaussuresKeys => $chaussuresValues) { ?>
                        <div class="card mb-2 mr-5 ml-5" style="width: 21rem; height: 23.5rem;">
                            <img src="<?php echo $chaussuresValues["product_img1"];?>" class="card-img-top" alt="...">
                            <div class="card-body">
                                <h5 class="card-title"><?php echo $chaussuresValues["product_title"];?></h5>
                            </div>
                            <ul class="list-group list-group-flush ">
                                <li class="list-group-item">$<?php echo $chaussuresValues["product_price"];?></li>
                            </ul>

                            <div class="card-body">
                                <a href="?page=details&detailsID=<?php echo $chaussuresValues["product_id"];?>" class="card-link homepage_product_details">Détails</a>
                                <a href="?addID=<?php echo $chaussuresValues["product_id"];?>" class="card-link bg-success homepage_add"><i class="fa fa-cart-plus fa-cart-plus_homepage" aria-hidden="true"></i>Ajouter Au Panier</a>
                            </div>
                        </div>
                    <?php } ?>
                ...
                <div style="margin-top: 40px; margin-left: 45px;" id="pagination">
                    <?php
                    $count = count($this->shopChaussures);
                    $num_per_page = 4;
                    ...
                    if ($pageLinks > 1) {
                        echo "<a href='?page=chaussures&pageLinks=" . ($pageLinks - 1) . "' class = mr-2>Précédente</a>";
                    }
                    for ($i = 1; $i <= $total_page; $i++) { ?>
                        <a href="?page=chaussures&pageLinks=<?php echo $i; ?>" class="mr-3"><?php echo $i; ?></a>
                    <?php } ?>
                    <?php
                    if ($i > $pageLinks and $pageLinks < $total_page) {
                        echo "<a href='?page=chaussures&pageLinks=" . ($pageLinks + 1) . "'class = mr-2>Suivant</a>";
                    } ?>
                </div>
            </section>
